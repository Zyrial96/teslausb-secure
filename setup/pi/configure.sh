...
function install_rc_local () {
    ...
    # Smart Retention Policy: Behalte letzte N Tage lokal
    echo "RETENTION_DAYS=\"${RETENTION_DAYS:-7}\"" >> ~/rc.local
...
function check_archive_configs () {
    ...
    case "$ARCHIVE_SYSTEM" in
        ...
        multi)
            ...
            check_variable "PRIMARY_BACKEND"
            check_variable "SECONDARY_BACKEND"
            
            # Validate backend types
            if [[ ! "$PRIMARY_BACKEND" =~ ^(cifs|rsync|rclone)$ ]]; then
                echo "STOP: Invalid PRIMARY_BACKEND: $PRIMARY_BACKEND (must be cifs, rsync, or rclone)"
                exit 1
            fi
            if [[ ! "$SECONDARY_BACKEND" =~ ^(cifs|rsync|rclone)$ ]]; then
                echo "STOP: Invalid SECONDARY_BACKEND: $SECONDARY_BACKEND (must be cifs, rsync, or rclone)"
                exit 1
            fi
            
            # Check primary backend config
            ...
            # Check secondary backend config
            case "$SECONDARY_BACKEND" in
                cifs)
                    # CIFS as secondary not recommended, but check if different server
