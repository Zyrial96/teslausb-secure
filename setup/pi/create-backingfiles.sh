...
# FIX: Validate campercent parameter
if [ -z "$CAM_PERCENT" ]; then
  echo "ERROR: CAM_PERCENT not specified. Usage: $0 <percent> <mountpoint>"
  exit 1
fi

if ! [[ "$CAM_PERCENT" =~ ^[0-9]+$ ]]; then
  echo "ERROR: CAM_PERCENT must be a number between 1 and 100"
  exit 1
fi

if [ "$CAM_PERCENT" -lt 1 ] || [ "$CAM_PERCENT" -gt 100 ]; then
  echo "ERROR: CAM_PERCENT must be between 1 and 100 (got: $CAM_PERCENT)"
  exit 1
fi
